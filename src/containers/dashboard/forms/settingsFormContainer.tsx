...
export default function SettingsFormContainer() {
    ...
    const settingsSchema = z.object({
        name: z.string().min(3, "O nome precisa ter no mínimo 3 caracters").default(store?.name || ""),
        currency: z.string().default(store?.currency || "usd"),
        description: z.string().default(store?.description || ""),
        keywords: z.string().default(store?.keywords || ""),
        maintenance: z.boolean().default(store?.maintenance || false),
    })
                ...
                <form onSubmit={handleSubmit(updateStore)}>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div>
                            <div>
                                ...
                                <Input {...register('name')} defaultValue={store?.name} />
                                {errors.name && <span className='text-destructive text-[12px]'>{errors.name.message}</span>}
                            </div>
                            <div>
                    <div className='mt-5'>
                                    <p className="mb-1">{t("setup.currency")}:</p>
                                    <Select {...register('currency')} onValueChange={(value) => setValue('currency', value,{shouldDirty: true})} defaultValue={store?.currency}>
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-3 mt-5'>
                        <div className='border rounded-md h-auto flex justify-between items-center p-5 sm:min-w-[400px]'>
                            ...
                            <Switch {...register('maintenance')} defaultChecked={store?.maintenance} onCheckedChange={(value) => setValue("maintenance", value)}/>
                        </div>
                    </div>
                    </div>
                        <div className="space-y-5">
                            <div>
                                ...
                                <Textarea className="resize-none" {...register('description')} defaultValue={store?.description} maxLength={60}/>
                            ...
                            <div>
                                ...
                                <Textarea className="resize-none" {...register('keywords')} defaultValue={store?.keywords} maxLength={60}/>
                                {errors.description && <span className='text-destructive text-[12px]'>{errors.description.message}</span>}
                            </div>
                        </div>
                    </div>
