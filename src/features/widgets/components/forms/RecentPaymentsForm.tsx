...
import RecentPaymentsWidgetSchema from "../../schema/RecentPaymentsSchema";
import SubHeaderSection from "../../../../components/commons/subHeader";


type RecentPaymentsFormProps = {
  initialData?: RecentPaymentsWidgetFormData,
  onSubmit: (data: RecentPaymentsWidgetFormData) => void,
  isPending: boolean,
}


export default function RecentPaymentsForm({isPending, onSubmit, initialData} : RecentPaymentsFormProps) {

  const { handleSubmit, register, formState: { errors, isDirty }, setValue, getValues } = useForm<RecentPaymentsWidgetFormData>({
    resolver: zodResolver(RecentPaymentsWidgetSchema),
...
<section>
      ...
      <SubHeaderSection title="Pagamentos recentes" description="Mude as opções de seu widget"/>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-end">
        <div>
          ...
          <Switch
            defaultChecked={getValues("config.showValue")}
            onCheckedChange={(checked) => setValue("config.showValue", checked, { shouldDirty: true })} // Atualiza o valor de 'visible'
            className="accent-primary"
          />
        </div>
      </div>
