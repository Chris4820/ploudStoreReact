...
export default function SettingsPage() {
    const settingsSchema = z.object({
        storeName: z.string().min(3, "O nome precisa ter no mínimo 3 caracters"),
        limit: z.string().optional(),
        logoImage: z.string().optional(),
        backgroundImage: z.string().optional(),
        faviconImage: z.string().optional()
    })
    ...
    const onSubmit = (data :any) => {
        ...
        const { storeName, usageLimit } = data;

        // Obtenha as URLs das imagens dos componentes ImageUpload
        const logoUrl = getValues('logoImage');
        const backgroundUrl = getValues('backgroundImage');
        const faviconUrl = getValues('faviconImage');
        ...
        // Exiba os valores dos campos e as URLs das imagens no console
        console.log('Nome da loja:', storeName);
        console.log('Limite de uso:', usageLimit);
        console.log('URL da Logo:', logoUrl);
        console.log('URL do Background:', backgroundUrl);
        console.log('URL do Favicon:', faviconUrl);

        // Aqui você pode realizar outras operações, como enviar os dados para o servidor
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div>
                            ...
                            {errors.storeName && <span className='text-destructive text-[12px]'>{errors.storeName.message}</span>}
                        </div>
                        <div>
                            <label htmlFor="usageLimit" className="block font-medium mb-1">Limite de uso</label>
                            <Input type='number' {...register('limit')} />
                            {errors.limit && <span className='text-destructive text-[12px]'>{errors.limit.message}</span>}
                        </div>
                    </div>

                    <div>
                    <div className='grid-cols-1 lg:grid-cols-2 mt-5'>
